...
class Streaming(CustomerSegmentationWithYolo):
    def __init__(self, in_source=None, out_source=None, fps=None, cam_fps=15, blur_strength=None, background="none"):
    ...
    def stream_video(self):
        ...
        try:
            ...
            print(f"Webcam({self.input_source}) lie fps not available. Set the fps accordingly. Exception: {e}")
        ...
        with pyvirtualcam.Camera(width=width, height=height, fps=self.fps) as cam:
            ...
            while self.running and cap.isOpened():
                ret, frame = cap.read()
                if not ret: # if we are not getting any frame, we will break it
                    break

                if frame_idx % frame_interval == 0:
                    # model inference by cv model
                    ...
                    if mask is not None:
                        if self.background == 'blur':
                            result_frame = self.apply_blur_with_mask(frame, mask, blur_strength=self.blur_strength ) # this function takes the frame that comes from our camera and the mask from our model
            ...
            cam.send(cv2.cvtColor(result_frame, cv2.COLOR_BGR2RGB)) # Convering the channel from bgr to rgb
    ...
    def list_available_devices(self):
        """
        This function list the video capturing devices available in the system
        """
        devices=[]
        for i in range(5):
